...
class MetricsCalculator:
    ...
    def hit_at_k(self, retrieved: List[str], relevant: List[str], k: int) -> float:
        ...
        # 只要前K个结果中有任何一个相关文档就返回1
        return 1.0 if any(doc in relevant_set for doc in retrieved_k) else 0.0
    ...
    def calculate_all_metrics(self, retrieved: List[str], relevant: List[str],
        ...
        for k in k_values:
            ...
            metrics[f'hit@{k}'] = self.hit_at_k(retrieved, relevant, k)
...
class SemanticMetrics:
    ...
    def crime_consistency_metrics(self, search_result: Dict) -> Dict[str, float]:
        ...
        if not isinstance(search_result, dict):
            return {
                'precision': 0.0, 'recall': 0.0, 'jaccard': 0.0,
                'case_coverage_rate': 0.0, 'covered_cases_count': 0,
                'total_cases_count': 0
            }
        ...
        # 案例级覆盖率计算 - 检查每个案例的引用法条是否在返回法条中
        covered_cases = 0  # 被覆盖的案例数
        total_cases = len(cases)
        case_cited_articles = set()  # 所有案例引用的法条（用于传统指标）
        ...
        for case in cases:
            ...
            if relevant_articles:
                ...
                # 检查这个案例的引用法条是否有部分在返回法条中
                case_articles_set = set(relevant_articles)
                if case_articles_set.intersection(returned_articles):
                    covered_cases += 1

        # 案例级覆盖率 - 核心指标
        case_coverage_rate = covered_cases / total_cases if total_cases > 0 else 0.0
        ...
        return {
            ...
            # 新的案例级指标 - 更重要
            'case_coverage_rate': case_coverage_rate,  # 案例级覆盖率
            'covered_cases_count': covered_cases,      # 被覆盖的案例数
            'total_cases_count': total_cases,          # 总案例数
            
            # 详细分析
            'returned_articles': sorted(list(returned_articles)),
            'case_cited_articles': sorted(list(case_cited_articles))
        }